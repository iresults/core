...
class CsvTranslationProvider extends AbstractFileBasedTranslationProvider {
	...
	protected function parseTranslationFile($translationFilePath) {
		...
		if ($fileHandle === FALSE) {
			throw new InvalidTranslationFileException('Couldn\'t load CSV file from ' . $translationFilePath, 1329750849);
		}

		while (($lineString = fgets($fileHandle)) !== FALSE && $line++ < $maxLines) {
			$row = str_getcsv($lineString);
			if (count($row) < 2) {
				throw new InvalidTranslationFileException('Error in input file ' . $translationFilePath . ': The row at line ' . $line . ' has less than two values', 1329750916);
			}
